    	...
    	---> Instrumentation.execStartActivity
    		---> IActivityManager.startActivity
    			...
    			---> AMS.startActivityAsUser
    					// ActivityStarter专门负责Activity的启动操作，包括解析Intent、创建ActivityRecord 、TaskRecord，并将Activity与Task以及Stack关联
    				---> ActivityStarter.startActivityMayWait
    				---> ActivityStarter.startActivityLocked
    				---> ActivityStarter.startActivity
    				---> ActivityStarter.startActivity
    				---> ActivityStarter.startActivityUnchecked
    					---> ActivityStackSupervisor.resumeFoucsedStackTopActivityLocked
    						---> ActivityStack.resumeTopActivityUncheckedLocked
    						---> ActivityStack.resumeTopActivityInnerLocked
    // 判断进程是否启动，未启动则通知zygote fork出新进程
    					---> ActivityStackSupervisor.startSpecificActivityLocked
    					---> ActivityStackSupervisor.realStartActivityLocked
    						---> ApplicationThread.realStartActivityLocked
    
    // 实例化ClientTransaction启动事务，构建LaunchActivityItem对象,并传入clientTransaction中,用作callback
ActivityStackSupervisor.realStartActivityLocked
    ---> ClientLifecycleManager.scheduleTransaction
    	---> ClientTransaction.schedule
    		---> ApplicationThread.scheduleTransaction
    		---> ActivityThread.sendMessage(EXECUTE_TRANSACTION, transaction)
    			---> LaunchActivityItem.execute
    				---> ActivityThread.handleLaunchActivity
    // 通过反射由Instrumentation.newActivity创建Activity
...
---> AMS.startProcess
	---> Process.start
// 启动主线程
// Looper.prepareMainLooper(), thread.attach(false, startSeq), Looper.loop()
ActivityThread.main
	---> thread.attach()
    ...
    // 构建ContextImpl，获取classLoader，构建Instrumentation，构建Application并回调其onCreate
